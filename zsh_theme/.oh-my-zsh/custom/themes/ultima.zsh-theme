...
char_badge="%F{white} on %f%F{white}${char_arrow}%f"
vc_branch_name="%F{#F54927}%b%f"

vc_action="%F{white}%a %f%F{white}${char_arrow}%f"
vc_unstaged_status="%F{cyan} M ${char_arrow}%f"

vc_git_staged_status="%F{green} A ${char_arrow}%f"
vc_git_hash="%F{green}%6.6i%f %F{white}${char_arrow}%f"
...
printPsOneLimiter() {
  ...
  echo $ANSI_dim_white$char_down_and_right_divider$spacing$ANSI_reset
}
...
PROMPT="%F{white}${char_up_and_right_divider} ${ssh_marker} %f%F{cyan}%~%f$(prepareGitStatusLine)
%F{#27F5CF} ${char_arrow}%f "
